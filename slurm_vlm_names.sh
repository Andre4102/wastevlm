...
# Describe-then-ask naming: one calibrated Yes/No question per category, asked
# with and without the model's own description in context.
#
# Usage:  SPLIT=test sbatch slurm_vlm_names.sh <ENCODER> <CKPT_DIR> [DATASET]
set -euo pipefail
...
DATASET=${3:-aw_m2}
SPLIT=${SPLIT:-test}
OUT=$WROOT/results/vlm_eval/names_$(basename "$CKPT_DIR")_${DATASET}_${SPLIT}.json
...
"$PYBIN/python" scripts/name_probe.py --generate \
